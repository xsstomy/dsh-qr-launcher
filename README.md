...
- **终端二维码**：`dsh web` 启动就绪后，终端自动打印局域网访问二维码（紧凑半块 ANSI 渲染，显式黑白显色、与终端主题无关，占用小）+ 明文 URL；
...
## 与 dsh-lan-access 的关系（是否依赖？）

**本插件不强制依赖、也不打包 dsh-lan-access**，它只负责"展示二维码"，不负责
"开启局域网监听"（按设计原则，网络绑定逻辑由其他插件负责）。

但要注意：**二维码要能真正扫码使用，前提是 webserver 已绑定 `0.0.0.0`**（DSH
默认仅监听 `127.0.0.1`，此时手机无法访问）。因此：

- ✅ 推荐：同时安装 [dsh-lan-access](https://www.npmjs.com/package/dsh-lan-access)（一键把 webserver 绑定到 `0.0.0.0`，本机当前即此方案）；
- ✅ 等价替代：在其他 bundle/自己的 `cordis.patch.yml` 中把 webserver 行的 `host`
  覆盖为 `0.0.0.0`（需同时保证 `/api` 信任栅栏认可访问来源，见 dsh-lan-access 文档）；
- ⚠️ 未绑定 `0.0.0.0` 时，本插件**不会打印无效二维码**，而是给出明确提示
  （终端与 Web 面板都会提示安装 dsh-lan-access 或覆盖 webserver 行 host）。

总之：**扫码可用 ≈ dsh-lan-access（或等效配置），展示二维码 = dsh-qr-launcher 本身**。
