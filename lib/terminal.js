...
/**
 * 渲染主题无关的 ANSI 二维码（每模块两空格 + 显式背景色）。
 * @param rows - '0'/'1' 字符串数组。
 * @param margin - 静区（模块数），默认 4。
 * @returns 可直接写终端的字符串。
 */
export function renderAnsiQr(rows, margin = 4) {
	...
	const lines = []
	const cell = (isDark) => (isDark ? BLACK_BG : WHITE_BG) + '  '
	const pad = () => WHITE_BG + '  '.repeat(margin)
	for (let i = 0; i < margin; i++) lines.push(WHITE_BG + '  '.repeat(width) + RESET)
	for (let r = 0; r < n; r++) {
		const line = rows[r]
		let row = pad()
		for (let c = 0; c < n; c++) row += cell(line[c] === '1')
		row += pad() + RESET
		lines.push(row)
	}
	for (let i = 0; i < margin; i++) lines.push(WHITE_BG + '  '.repeat(width) + RESET)
